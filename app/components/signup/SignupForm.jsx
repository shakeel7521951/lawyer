...
export default function SignupForm() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-30 px-4 sm:px-8 md:px-12 lg:px-16 sm:py-36 bg-[#494c52]">
      <div className="w-full max-w-4xl bg-white shadow-2xl rounded-3xl overflow-hidden border border-[#c0b688]">

        {/* Header */}
        <div className="bg-[#c0b688] py-5 px-4 sm:px-8 text-center">
          <h1 className="text-[#9f8660] font-serif text-2xl sm:text-3xl font-bold">Al Khaldi Law Firm</h1>
          <p className="text-white text-sm mt-2 tracking-wide">Trusted Legal Excellence Since 2004</p>
        </div>

        {/* Form Body */}
        <div className="p-6 sm:p-10">
          <h2 className="text-2xl sm:text-3xl font-serif font-bold text-[#494c52] mb-8 text-center">
            Create Your Account
          </h2>
          ...
          {[
            ...
            <div className="mb-5" key={idx}>
              <label className="block text-[#494c52] font-semibold mb-2">{label}</label>
              <div className="relative">
                <Icon className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#494c52]" size={18} />
                <input
                  type={type}
          ...
          <div className="text-center mt-4 text-[#0D1B2A]">
            Already have an account?{" "}
            <Link href="/login" className="text-[#494c52] hover:text-[#9f8660] hover:underline font-medium">
              Login
            </Link>
          </div>
        </div>
